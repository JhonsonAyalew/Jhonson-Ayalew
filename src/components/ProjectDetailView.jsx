            ...
            <div className="glass-card" style={{ padding:'14px', marginTop:'8px' }}>
              ...
              <div style={{ display:'flex', flexWrap:'wrap', gap:'5px' }}>
                {project.details.tech.map(t => (
                  <span key={t} style={{ padding:'3px 9px', background:'rgba(0,180,255,0.08)', border:'1px solid rgba(0,180,255,0.2)', fontSize:'0.52rem', color:'var(--cyan)', letterSpacing:'0.04em' }}>{t}</span>
                ))}
              </div>
            </div>
            <button onClick={onBack} data-cursor
              style={{ marginTop:'10px', background:'none', border:'1px solid var(--border-subtle)', color:'var(--text-muted)', fontFamily:'Share Tech Mono', fontSize:'0.52rem', letterSpacing:'0.14em', padding:'8px 16px', cursor:'none', transition:'all 0.25s' }}
              ...
              <div style={{ marginTop:'18px', paddingTop:'14px', borderTop:'1px solid var(--border-subtle)' }}>
                ...
                {project.details.highlights.map((h, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'8px', padding:'5px 0', fontSize:'0.58rem', color:'var(--text-muted)' }}>
                    <span style={{ color:'var(--cyan-dim)', flexShrink:0 }}>→</span>{h}
                  </div>
                ))}
              </div>
            )}
