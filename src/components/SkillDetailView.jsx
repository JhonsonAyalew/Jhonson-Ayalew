          ...
          <motion.div key={skill.id} variants={slideLeft} initial="hidden" animate="visible" exit="exit">
            ...
            <div className="glass-card" style={{ padding: '26px', marginBottom: '10px', borderRadius: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
                <div style={{ width: '9px', height: '9px', borderRadius: '50%', background: 'var(--primary-white)', boxShadow: '0 0 12px rgba(255,255,255,0.4)' }} />
                ...
                <div style={{ position: 'relative', width: '70px', height: '70px', flexShrink: 0 }}>
                  <svg width="70" height="70" viewBox="0 0 70 70">
                    ...
                    <motion.circle cx="35" cy="35" r="28" fill="none"
                      stroke="url(#monoGrad)" strokeWidth="5" strokeLinecap="round"
                      strokeDasharray={`${2 * Math.PI * 28}`}
                      initial={{ strokeDashoffset: 2 * Math.PI * 28 }}
                      animate={{ strokeDashoffset: 2 * Math.PI * 28 * (1 - skill.level / 100) }}
                      transition={{ duration: 1.2, ease: [0.4, 0, 0.2, 1] }}
                      style={{ transform: 'rotate(-90deg)', transformOrigin: '50% 50%', filter: 'drop-shadow(0 0 6px rgba(255,255,255,0.35))' }}
                    />
                    <defs>
                      <linearGradient id="monoGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                        <stop offset="0%" stopColor="#8a8a8a" /><stop offset="100%" stopColor="#ffffff" />
                      </linearGradient>
                    </defs>
                  </svg>
              ...
              <div className="glass-card" style={{ padding: '16px', marginBottom: '10px', borderRadius: '12px' }}>
                ...
                {relatedProjects.map(p => (
                  <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 0', borderBottom: '1px solid var(--border-subtle)', fontSize: '0.72rem', color: 'var(--text-secondary)', fontFamily: 'Inter' }}>
                    <span style={{ color: 'var(--text-light)' }}>◆</span>{p.title}
                  </div>
                ))}
              </div>
            )}
        ...
        <motion.div variants={slideRight}>
          <div className="glass-card" style={{ padding: '26px', maxHeight: '460px', overflowY: 'auto', borderRadius: '14px' }}>
            <div style={{ marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: 'linear-gradient(135deg,var(--primary-white),var(--primary-mid))', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Sparkles size={13} color="var(--bg-deep)" strokeWidth={2.5} />
              </div>
              <span style={{ fontSize: '0.55rem', letterSpacing: '0.18em', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Johnson AI — {skill.name}</span>
            </div>
            {aiContent ? (
