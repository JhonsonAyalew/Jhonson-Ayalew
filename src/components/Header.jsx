  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      for (const link of navLinks) {
        ...
        if (!el) continue
        if (el.getBoundingClientRect().top <= window.innerHeight * 0.4) current = link.href
      }
      setActive(current)
    }
  ...
  return (
    <header className="anim-fade-up fixed inset-x-0 top-0 z-50 bg-black/70 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4 lg:px-8">
        ...
        <a
          href="#"
          className="inline-flex items-center gap-2 rounded-md border border-white/15 px-4 py-2 text-sm font-medium text-[#d6d6d6] transition-all duration-300 hover:-translate-y-px hover:border-white/35 hover:text-white"
        >
